...
# run our regression model
ever.wk <- glm(everwrk~age_p+r_maritl,
               data=NH11, family="binomial")
coef(summary(ever.wk))

ever.wk.tab <- coef(summary(ever.wk))
ever.wk.tab[, "Estimate"] <- exp(coef(ever.wk))
ever.wk.tab

#Plotting
plot(allEffects(ever.wk))


#Predicting
predDat.wk <- with(NH11,
                expand.grid(
                            r_maritl = unique(NH11$r_maritl),
                            age_p = mean(age_p,na.rm=TRUE)))
cbind(predDat.wk, predict(ever.wk, type = "response",
                       se.fit = TRUE, interval="confidence",
                       newdata = predDat.wk))
